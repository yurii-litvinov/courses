...
\section{Замыкания ``под капотом''}

Есть некоторая тонкость, связанная с замыканиями и локальными переменными:
...
Выведется 10 раз число 10, поскольку в замыкание попадёт ссылка на локальную переменную цикла i, которая в тот момент, когда делегаты начнут вызываться, уже будет иметь значение 10. Может быть слегка не интуитивно. Тем не менее, если взять какой-нибудь декомпилятор (например, dotPeek), сказать ему показывать сгенерированный код и декомпилировать эту программу, мы увидим объяснение (тут код приводится с пропуском некоторых ненужных подробностей типа пустого конструктора):
...
Для замыкания сгенерировался отдельный класс с полем i, объект этого класса создаётся перед циклом, в цикле счётчиком стало это самое поле i из нового класса, так что не удивительно, что после цикла оно равно 10. Давайте немного поменяем программу:
