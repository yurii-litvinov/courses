...
\slidetitle{Паттерны и архитектурные стили}{13.07.2026}
...
\begin{document}
    ...
    \begin{frame}{Архитектурные стили}
        ...
        \end{enumerate}
        Архитектурные шаблоны более \enquote{стратегичны} и более размыты, чем паттерны
    \end{frame}

    \begin{frame}{Паттерны и архитектурные стили}
        \begin{center}
    ...
    \begin{frame}{Книжка про паттерны}
        \begin{columns}
    ...
    \begin{frame}{Пример: Model-View-Controller}
        \begin{center}
        ...
        \begin{itemize}
            ...
            \begin{itemize}
                \item Естественное место для паттерна \enquote{Команда} и Undo/Redo
            \end{itemize}
        \end{itemize}
    \end{frame}
    ...
    \subsection{Паттерн \enquote{Команда}}

    \begin{frame}{Паттерн \enquote{Команда}, мотивация}
        \begin{itemize}
            \item Хотим отделить инициацию запроса от его исполнения
            \item Хотим, чтобы тот, кто \enquote{активирует} запрос, не знал, как он исполняется
    ...
    \begin{frame}{\enquote{Команда} (Command), детали реализации}
        \begin{itemize}
            \item Насколько \enquote{умной} должна быть команда
            \item Отмена и повторение операций --- тоже от хранения всего состояния в команде до \enquote{вычислимого} отката
            \begin{itemize}
...
fileToolBar->addAction(openAct);
    ...
    \subsection{Паттерн \enquote{Абстрактная фабрика}}
    ...
    \begin{frame}{\enquote{Абстрактная фабрика}, детали реализации}
        \begin{columns}
            \begin{column}{0.5\textwidth}
                \begin{itemize}
                    \item Хорошо комбинируются с паттерном \enquote{Одиночка}
    ...
    \subsection{Паттерн \enquote{Заместитель}}

    \begin{frame}{Управление доступом к объектам}
        \begin{itemize}
    ...
    \begin{frame}{Отложенная загрузка изображения}
        \begin{center}
            \includegraphics[width=0.7\textwidth]{proxyExampleClassDiagram.png}
        \end{center}
    \end{frame}

    \begin{frame}{Паттерн \enquote{Заместитель}}
        \framesubtitle{Proxy}
        \begin{center}
        ...
        \begin{itemize}
            ...
            \item Создание \enquote{тяжёлых} объектов по требованию
    ...
    \begin{frame}{\enquote{Заместитель}, детали реализации}
        \begin{center}
            \includegraphics[width=0.6\textwidth]{proxy.png}
        \end{center}
        \begin{itemize}
    ...
    \begin{frame}{\enquote{Заместитель}, детали реализации (2)}
        \begin{itemize}
            \item Реализация \enquote{вручную} всех методов проксируемого объекта
            \begin{itemize}
    ...
    \subsection{Паттерн \enquote{Итератор}}

    \begin{frame}{\enquote{Итератор} (Iterator)}
    ...
    \begin{frame}[fragile]
        \frametitle{\enquote{Итератор}, примеры}
        \begin{itemize}
            \item Java-стиль:
            \begin{minted}{java}
...
public interface IEnumerator<T>
{
    ...
    \begin{frame}[fragile]{\enquote{Итератор}, детали реализации (1)}
        \begin{itemize}
...
collection.ToList().ForEach(t => Console.WriteLine(t));
    ...
    \begin{frame}{\enquote{Итератор}, детали реализации (2)}
        \begin{itemize}
    ...
    \begin{frame}{Слоистый стиль}
        \framesubtitle{Layered style}
        \begin{itemize}
            \item Иерархическая организация системы
            \begin{itemize}
                \item Многоуровневый \enquote{клиент-сервер}
    ...
    \begin{frame}{Слоистый стиль, подробности}
        \begin{columns}
            \begin{column}{0.5\textwidth}
                \begin{itemize}
                    \item Преимущества:
    ...
    \begin{frame}{Каналы и фильтры, подробности}
        \begin{itemize}
            \item Преимущества:
            \begin{itemize}
            ...
            \begin{itemize}
                ...
                \item Пропускная способность определяется самым \enquote{узким} элементом
            \end{itemize}
        \end{itemize}
    \end{frame}
    ...
    \begin{frame}{Стили с неявным вызовом}
        \begin{itemize}
    ...
    \begin{frame}{Стили с неявным вызовом, преимущества и недостатки}
        \begin{itemize}
            \item Преимущества:
            \begin{itemize}
