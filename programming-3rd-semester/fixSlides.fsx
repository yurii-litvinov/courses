...
for dir in dirs do
    ...
    for file in texFiles do
        ...
        let targetRegex = Regex(@"\\documentclass.*{beamer}.*\\title{(.*)}.*\\author.*\\date{(.*)}.*\\begin{document}", RegexOptions.Singleline)
        ...
        for aMatch in matches do
            let newText = $"\\documentclass{{../../slides-style}}\n\n\\slidetitle{{{aMatch.Groups[1]}}}{{{aMatch.Groups[2]}}}\n\n\\begin{{document}}"
