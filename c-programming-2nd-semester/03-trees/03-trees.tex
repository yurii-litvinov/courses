...
\begin{document}
    ...
    \begin{frame}{Летучка}
        \begin{enumerate}
            \item Опишите хотя бы два режима работы с файлами в Си и укажите, как использовать данные режимы с функцией fopen
            \item Опишите, зачем нужна виртуальная память
            \item Напишите программу, которая приведёт память в состояние, представленное ниже
            \begin{outline}
                \1 Считаем, что куча --- это непрерывная область памяти
                \1 Единица адресации (как и ячейка) --- 4 байта
                \1 Аллокатор всегда работает по стратегии first fit
                \1 Внутренний заголовок блока всегда равен 4 байт
                \1 Пренебрегаем выравниваем
            \end{outline}
        \end{enumerate}
        \vspace{5mm}
        \begin{center}
            \includegraphics[width=0.95\textwidth]{memory-layout.png}
        \end{center}
    \end{frame}
