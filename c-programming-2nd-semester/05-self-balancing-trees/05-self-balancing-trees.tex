...
\slidetitle{Самобалансирующиеся деревья}{28.02.2026}
...
\begin{document}
    ...
    \begin{frame}{Летучка}
        \begin{enumerate}
            \item Сколько памяти занимает структура узла бинарного дерева? С учётом/без учёта выравнивания.
            \item Известно, что файловая система выражается через дерево. Свяжите термины файловой системы (файл, директория, корневая директория, родительская директория) с терминами дерева (корень, лист, узел и тд). Как бы изменилась файловая система, если бы она выражалась через бинарное дерево?
            \item Напишите функцию isLeaf(n) --- инвариант для проверки, что узел является листом.
            \item Какая операция является более трудоëмкой для несбалансированного бинарного дерева поиска: вставка или удаление? Ответ поясните.
        \end{enumerate}
        Каждая задача оценивается в один балл, но всего за работу можно получить не более трёх баллов
    \end{frame}
